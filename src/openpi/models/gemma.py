...
class Block(nn.Module):
    ...
    def __call__(
        ...
        star_ctx: _star_controller.JaxStarContext | None = None,
        star_params: _star_controller.JaxStarParams | None = None,
    ):
        star_state: _star_controller.JaxStarState | None = None
        ...
        if star_state is not None and star_ctx is not None and star_params is not None:
            expert_idx = int(star_params.action_expert_index)
            if 0 <= expert_idx < len(xs) and residual_inputs[expert_idx] is not None and xs[expert_idx] is not None:
                star_state, corrected = _star_controller.jax_star_step(
                    star_state,
                    residual_inputs[expert_idx],
                    xs[expert_idx],
                    star_ctx,
                    star_params,
                )
                xs = list(xs)
                xs[expert_idx] = corrected
        if star_state is None:
            return xs, kv_cache
        return (xs, star_state), kv_cache
...
class Module(nn.Module):
    ...
    def setup(self):
        ...
        self.layers = nn.scan(
            ...
            in_axes=(
                ...
                nn.broadcast,
                nn.broadcast,
            ),  # 0=kv_cache, 1=positions, 2=mask, 3=adarms_cond, 4=deterministic, 5=star_ctx, 6=star_params
    ...
    def __call__(
        ...
        *,
        ...
        KVCache,
        _star_controller.JaxStarState | None,
    ]:
        ...
        carry = embedded if star_state is None else (embedded, star_state)

        carry, kv_cache = self.layers(
            carry,
            kv_cache,
            positions,
            mask,
        ...
        if star_state is None:
            embedded = carry
            next_star_state = None
        else:
            embedded, next_star_state = carry
