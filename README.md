...
This repository is a release-oriented implementation of **STAR inference for
$\pi_{0.5}$ on LIBERO-PRO**. It is based on OpenPI and keeps the native OpenPI
training, checkpoint-loading, policy-server, and LIBERO evaluation paths. STAR
is training-free: the same `pi05_libero` checkpoint can be served in either
`NATIVE` or `STAR` mode.

The paper setting included here uses:

- a continuous $\beta_t$ range of `0.25` to `0.55`;
- STAR correction in the final 6 of the 18 action-expert blocks;
- a 2-second causal proprioceptive-statistics window;
- LIBERO control at 10 Hz with replanning every 5 environment steps.

Checkpoints, datasets, virtual environments, caches, experiment artifacts, and
machine-specific launch scripts are intentionally not distributed. See the
[STAR LIBERO-PRO guide](docs/star_libero_pro.md) for native/STAR commands and
the expected LIBERO-PRO suites.
...
```bash
git clone --recurse-submodules <repository-url> openpi-star
cd openpi-star
